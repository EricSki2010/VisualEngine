...
## Math, Image Loading & Text
- **GLM** — header-only math library (installed via vcpkg)
- **stb_image** — single-header image loader for textures (installed via vcpkg)
- **FreeType 2** — font rasterization for text rendering (installed via vcpkg)

## Package Manager
- **vcpkg** — used for GLM, stb_image, and FreeType (`C:/vcpkg/installed/x64-windows/`)
...
3. Install vcpkg, then: `vcpkg install glm stb freetype:x64-windows`
