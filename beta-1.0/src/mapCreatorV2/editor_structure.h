...
struct StructureEditor {
    ...
    void handleImportInput() {
        if (!importOpen || importPreview) return;
    ...
    void updateCameraWASD(float dt, EditorCamera& editorCam) {
    ...
    void drawScene(EditorCamera& editorCam, PlacedBlocks& placedBlocks, GradientSky& sky) {
        ...
        BeginMode3D(editorCam.cam);
            ...
            if (importPreview) {
                drawImportPreview(placedBlocks);
            }
        EndMode3D();
    ...
    void drawImportPreview(PlacedBlocks& placedBlocks) {
        previewBlocks.forEach([&](int x, int y, int z, const std::string& blockName) {
            Vec3i w = placedBlocks.toWorld({x, y, z});
            Vector3 v = { (float)w.x, (float)w.y, (float)w.z };
            std::string baseName = stripRotationSuffix(blockName);
            int yRot = getYRotation(blockName);
            ...
            if (bIt != blocks.entries.end()) {
                ...
                if (def.formatType == "glb" && def.modelLoaded) {
                    ...
                    DrawModelEx(def.model, mp, {0,1,0}, yRot * 90.0f,
    ...
    bool drawEscMenu(std::string& savePath, std::string& structureName,
        ...
        if (drawMenuButton("Import Structure", bx, by+bh+gap, bw, bh)) {
            ...
            importOpen = true;
            {
                ...
                importEntries.clear();
                for (auto& e : all)
                    if (e.type == "3D Structure") importEntries.push_back(e);
            }
            selectedImport = -1;
    ...
    void drawOriginConfirm(EditorCamera& editorCam, PlacedBlocks& placedBlocks) {
        int sW = GetScreenWidth(), sH = GetScreenHeight();
        DrawRectangle(-50, -50, sW+100, sH+100, {0,0,0,160});
        ...
        if (drawMenuButton("Yes", bsx+bw+bg, by, bw, bh)) {
            ...
            ChunkMatrix remapped;
            placedBlocks.matrix.forEach([&](int x, int y, int z, const std::string& bn) {
                remapped.set(x + delta.x, y + delta.y, z + delta.z, bn);
            });
            placedBlocks.matrix = std::move(remapped);
    ...
    void drawImportOverlay(PlacedBlocks& placedBlocks) {
        int sW = GetScreenWidth(), sH = GetScreenHeight();
        DrawRectangle(-50, -50, sW+100, sH+100, {0,0,0,160});
        ...
        drawImportSaveList(lx, lw, ly, ih, mv);
        int cy = drawImportPositionFields(lx, lw, ly, ih, mv);
        int ty = cy + 24;
        drawImportReplaceToggle(lx, ty);
        ...
        if (drawMenuButton("Preview", bsx+bw+bg, by, bw, bh, ci)) {
            if (ci) {
                ...
                previewBlocks.clear();
                ib.forEach([&](int x, int y, int z, const std::string& bn) {
                    previewBlocks.set(bo.x+x, bo.y+y, bo.z+z, bn);
                });
                importPreview = true;
            }
        }
        ...
        if (drawMenuButton("Import", bsx+2*(bw+bg), by, bw, bh, ci)) {
            if (ci) {
                ...
                Vec3i bo = placedBlocks.toOffset({tx,ty2,tz});
                ib.forEach([&](int x, int y, int z, const std::string& bn) {
                    Vec3i fo = {bo.x+x, bo.y+y, bo.z+z};
                    if (importReplaceBlocks || !placedBlocks.matrix.has(fo.x, fo.y, fo.z))
                        placedBlocks.matrix.set(fo.x, fo.y, fo.z, bn);
                });
                placedBlocks.markDirty();
    ...
    void drawImportSaveList(int lx, int lw, int ly, int ih, int mv) {
    ...
    int drawImportPositionFields(int lx, int lw, int ly, int ih, int mv) {
        ...
        for (int f = 0; f < 3; f++) {
            ...
            if (f==importFocusField && ((int)(GetTime()*2.0)%2)==0) {
                int cx2 = ix+6+MeasureText(flds[f]->c_str(), 20);
                DrawRectangle(cx2, cy+6, 2, 24, WHITE);
            }
            Vector2 mouse = GetMousePosition();
    ...
    void drawImportReplaceToggle(int lx, int ty) {
        int bs = 20;
    ...
    bool frame(float dt, EditorCamera& editorCam, PlacedBlocks& placedBlocks,
        ...
        BeginDrawing();
            ...
            drawScene(editorCam, placedBlocks, sky);
            placedBlocks.drawStructDragRect();

            if (escMenuOpen)        exitToMenu = drawEscMenu(savePath, structureName,
                                                              structureType, placedBlocks, editorCam);
            if (originConfirmOpen)  drawOriginConfirm(editorCam, placedBlocks);
            if (importOpen && !importPreview) drawImportOverlay(placedBlocks);
        EndDrawing();
